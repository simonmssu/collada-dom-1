...
daeElementRef
domJoint::create(daeInt)
{
	domJointRef ref = new domJoint;
	return ref;
}


daeMetaElement *
domJoint::registerElement()
{
    if ( _Meta != NULL ) return _Meta;
    
    _Meta = new daeMetaElement;
    _Meta->setName( "joint" );
	_Meta->registerClass(domJoint::create, &_Meta);
    ...
    _Meta->addContentsOrder(daeOffsetOf(domJoint,_contentsOrder));
        
    _Meta->addCMDataArray(daeOffsetOf(domJoint,_CMData), 1);
	//	Add attribute: id
 	{
	...
	_Meta->setElementSize(sizeof(domJoint));
	_Meta->validate();
...
daeMetaElement * domJoint::_Meta = NULL;
