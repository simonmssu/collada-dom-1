 ...
 * License. 
 */
#ifndef __domFaces_h__
#define __domFaces_h__
...
{
public:
	COLLADA_TYPE::TypeEnum getElementType() const { return COLLADA_TYPE::FACES; }
public:
	class domVcount;
 ...
 * surface and one for referencing the wires themselfs. @see domInput
 */
	domInputLocalOffset_Array elemInput_array;
	domVcountRef elemVcount;
/**
 *  The indeces for referencing the surface and wires for each face.  @see
 * domP
 */
	domPRef elemP;
...
protected:
	 ...
	 * Constructor
	 */
	domFaces() : attrId(), attrName(), attrCount(), elemInput_array(), elemVcount(), elemP() {}
	/**
	 * Destructor
	 */
	virtual ~domFaces() {}
	 ...
	 * Copy Constructor
	 */
	domFaces( const domFaces &cpy ) : daeElement() { (void)cpy; }
	/**
	 * Overloaded assignment operator
	 */
