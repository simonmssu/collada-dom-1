...
{
public:
	COLLADA_TYPE::TypeEnum getElementType() const { return COLLADA_TYPE::SOLIDS; }
public:
	class domVcount;
...
protected:  // Elements
/**
 * One input elements is needed to define a solid by its shells. @see domInput
 */
	domInputLocalOffset_Array elemInput_array;
	domVcountRef elemVcount;
/**
 *  The indeces of the shells for each solid.  @see domP
 */
	domPRef elemP;
...
protected:
	 ...
	 * Constructor
	 */
	domSolids() : attrId(), attrName(), attrCount(), elemInput_array(), elemVcount(), elemP() {}
	/**
	 * Destructor
	 */
	virtual ~domSolids() {}
	 ...
	 * Copy Constructor
	 */
	domSolids( const domSolids &cpy ) : daeElement() { (void)cpy; }
	/**
	 * Overloaded assignment operator
	 */
