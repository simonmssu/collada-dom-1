...
/**
 * Primitive element. @brief The first two indices form a line.  @brief Each
 * subsequent index extends the line from the previous index. Each p element
...
/**
 *  The count attribute indicates the number of linestip primitives.  Required
 * attribute. 
 */
	xsNonNegativeInteger attrCount;
/**
 *  The material attribute refers to the name of a material element, using
 * a URL  expression, bound to the linestrip. If the material attribute is
...
protected:
	 ...
	 * Constructor
	 */
	domLinestrips() : attrCount(), attrMaterial(), elemParam_array(), elemInput_array(), elemP_array() {}
	/**
	 * Destructor
	 */
