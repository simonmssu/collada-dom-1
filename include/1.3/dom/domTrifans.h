...
/**
 * The trifans element declares the binding of geometric primitives and vertex
 * attributes for a  mesh element. The trifans element provides the information
 * needed to bind vertex attributes  together and then organize those vertices
 * into connected triangles. The vertex array  information is supplied in
 * distinct attribute arrays of the mesh element that are then  indexed by
 * the trifans element. Each triangle described by the mesh has three vertices.
 * The  first triangle is formed from first, second, and third vertices. Each
 * subsequent triangle is  formed from the current vertex, reusing the first
 * and previous vertices. A trifans element  contains a sequence of p elements,
 * where 'p' stands for primitive. Each p element describes  the vertex attributes
 * for an arbitrary number of connected triangles.
 */
 ...
 * Each subsequent index forms an additional triangle reusing the first and
 * previous indices. Each p element contains indices that reference into the
 * source elements. These  indices are position dependent and reference the
 * contents of the source elements according to the order of the input elements.
 * Here is an example of this: The  first index references the first unique
 * input element; the second index references  the second unique input element,
...
/**
 *  The count attribute indicates the number of trifan primitives.  Required
 * attribute. 
 */
	xsNonNegativeInteger attrCount;
/**
 *  The material attribute refers to the name of a material element, using
 * a URL  expression, bound to the trifans. If the material attribute is not
...
protected:
	 ...
	 * Constructor
	 */
	domTrifans() : attrCount(), attrMaterial(), elemParam_array(), elemInput_array(), elemP_array() {}
	/**
	 * Destructor
	 */
	virtual ~domTrifans() {}
	 ...
	 * Copy Constructor
	 */
	domTrifans( const domTrifans &cpy ) : daeElement() { (void)cpy; }
	/**
	 * Overloaded assignment operator
	 */
